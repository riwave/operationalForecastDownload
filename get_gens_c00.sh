...
HGT=":HGT:850 mb:"
VARSGETGEFS="$UGRD|$VGRD|$GUST|$PRMSL|$HGT"
  ...
  for e in $ensbl;do
    ...
    while [ $TAM -lt 800000 ] && [ $TRIES -le 130 ]; do
      ...
      if [ ${TAM} -lt 800000 ]; then
          ...
          if [ ${e} == 00 ]; then
             $DIRS/get_inv.pl $SERVER/data/nccf/com/gens/prod/gefs.$ANO$MES$DIA/$HORA/pgrb2ap5/gec${e}.t${HORA}z.pgrb2a.0p50.f"$(printf "%03d" $h)".idx | egrep "($VARSGETGEFS)" | $DIRS/get_grib.pl $SERVER/data/nccf/com/gens/prod/gefs.$ANO$MES$DIA/$HORA/pgrb2ap5/gec${e}.t${HORA}z.pgrb2a.0p50.f"$(printf "%03d" $h)" $DIR/gens.$ANO$MES$DIA$HORA/gefs${e}.t${HORA}z.pgrb2f"$(printf "%03d" $h)".grb2 >> $DIR/gens.$ANO$MES$DIA$HORA/logGENS_$ANO$MES$DIA$HORA 2>&1         
          else
             $DIRS/get_inv.pl $SERVER/data/nccf/com/gens/prod/gefs.$ANO$MES$DIA/$HORA/pgrb2ap5/gep${e}.t${HORA}z.pgrb2a.0p50.f"$(printf "%03d" $h)".idx | egrep "($VARSGETGEFS)" | $DIRS/get_grib.pl $SERVER/data/nccf/com/gens/prod/gefs.$ANO$MES$DIA/$HORA/pgrb2ap5/gep${e}.t${HORA}z.pgrb2a.0p50.f"$(printf "%03d" $h)" $DIR/gens.$ANO$MES$DIA$HORA/gefs${e}.t${HORA}z.pgrb2f"$(printf "%03d" $h)".grb2 >> $DIR/gens.$ANO$MES$DIA$HORA/logGENS_$ANO$MES$DIA$HORA 2>&1         
          fi
...
for e in $ensbl;do
  /usr/bin/ncecat $DIR/gens.$ANO$MES$DIA$HORA/gefs${e}.t${HORA}z.pgrb2f*.nc -O $DIR/gens.$ANO$MES$DIA$HORA/gefs.$ANO$MES$DIA$HORA.m${e}.pgrb2ap5.nc
